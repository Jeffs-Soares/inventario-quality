...
<main class="flex-1 p-4 sm:p-6 lg:p-8">
   ...
   <div class="max-w-4xl mx-auto bg-white p-6 rounded-lg shadow-md mt-8">
                    <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 mb-6">Editar Registro</h1>

                    <form action="{{ route('registro.update', $registro->id) }}" method="post" class="space-y-6">
                        @csrf
                        @method('put')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            <!-- Nova Categoria -->
                            <div>
                                <label for="categoria" class="block text-sm font-medium text-gray-700 mb-1">Nova Categoria:</label>
                                <select id="categoria" name="categoria" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md shadow-sm">
                                    @foreach($categorias as $categoria)
                                        <option value="{{ $categoria->id }}" @if(isset($registro) && $registro->categoria_id == $categoria->id) selected @endif>
                            ...
                            <!-- Novo Item -->
                            <div>
                                <label for="item" class="block text-sm font-medium text-gray-700 mb-1">Novo Item:</label>
                                <select id="item" name="item" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md shadow-sm">
                                    @foreach($items as $item)
                                        <option value="{{ $item->id }}" @if(isset($registro) && $registro->item_id == $item->id) selected @endif>
                                            {{ $item->item_descricao }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <!-- Data da Aquisição -->
                            <div>
                                <label for="data_aquisicao" class="block text-sm font-medium text-gray-700 mb-1">Data da Aquisição</label>
                            ...
                            <div>
                                <label for="local" class="block text-sm font-medium text-gray-700 mb-1">Selecione o novo Local:</label>
                                <select id="local" name="local" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md shadow-sm">
                                    @foreach($locais as $local)
                                        <option value="{{ $local->id }}" @if(isset($registro) && $registro->local_id == $local->id) selected @endif>
                                            {{ $local->loc_descricao }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="flex flex-col sm:flex-row mt-6 gap-2">
                            ...
                            <a href="{{route('registro.index')}}" class="w-full sm:w-auto text-center text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-red-600 dark:hover:bg-red-700 focus:outline-none dark:focus:ring-red-800">
                                Cancel
                            </a>
                        </div>
                    </form>
