...
<div class="max-w-4xl mx-auto bg-white p-6 rounded-lg shadow-md mt-8">
        <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 mb-6">Editar Local</h1>

        <form action="{{route('local.update', $local->id)}}" method="post" class="space-y-6">
            @csrf
            @method('put')

            <div>
                <label for="loc_descricao" class="block text-sm font-medium text-gray-700 mb-1">Nome do Local</label>
            ...
            <div class="flex flex-col sm:flex-row mt-6 gap-2">
                ...
                <a href="{{ route('local.index') }}" class="w-full sm:w-auto text-center text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-red-600 dark:hover:bg-red-700 focus:outline-none dark:focus:ring-red-800">
                    Cancel
                </a>
            </div>
        </form>
